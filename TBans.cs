...
public partial class TBans : BasePlugin, IPluginConfig<ConfigBan>
{
    public override string ModuleName => "TBans";
    public override string ModuleAuthor => "DeadSwim";
    public override string ModuleDescription => "Banning players to join in T.";
    ...
    public HookResult OnPlayerConnect(EventPlayerConnectFull @event, GameEventInfo info)
    {
        ...
        if(CheckBan(player) == true)
        {
            ...
            if (GetPlayerBanTime(player) < nowtimeis)
            {
                ...
                MySql.Table("deadswim_tbans").Where($"ban_steamid = '{steamid}'").Delete();
                banned[client] = false;
    ...
    public HookResult OnPlayerChangeTeam(CCSPlayerController? player, CommandInfo command)
    {
        ...
        if(player_team == 2)
        {
            ...
            {
                player.PrintToChat($" {Config.Prefix} You are banned to connect in {ChatColors.Darkred}T{ChatColors.Default}.");
                player.PrintToChat($" {Config.Prefix} Remaining of Ban to {ChatColors.Darkred}T{ChatColors.Default} is {ChatColors.Red}{remaining[client]}.");
