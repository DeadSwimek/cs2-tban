...
public partial class TBans
{
    ...
    public void CreateDatabase()
    {
        try
        {
            ...
            MySql.ExecuteNonQueryAsync(@"CREATE TABLE IF NOT EXISTS `deadswim_tbans` (`id` INT AUTO_INCREMENT PRIMARY KEY, `ban_steamid` VARCHAR(32) UNIQUE NOT NULL, `end` INT(11) NOT NULL, `reason` VARCHAR(32) UNIQUE NOT NULL, `banned_by` VARCHAR(32) UNIQUE NOT NULL, UNIQUE (`ban_steamid`));");
    ...
    public bool CheckBan(CCSPlayerController? player)
    {
        ...
        MySqlQueryResult result = MySql!.Table("deadswim_tbans").Where(MySqlQueryCondition.New("ban_steamid", "=", player.SteamID.ToString())).Select();
        if (result.Rows == 1)
        {
            return true;
        }
